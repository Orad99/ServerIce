...
c=conn.cursor()
c.execute("SELECT * FROM serversData WHERE serverNumber=:serverNumber", {'serverNumber': 21})
ON = c.fetchone()[1]
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        text="Dry-ice machine in servers-room"
    ...
    def update_btn_state(self, text):
        ...
        if ON == "open":
            self.ePushtbtn.setStyleSheet("background-color: gray")
            self.mc.publish_to("IOT_PROJECT","Close the Dry-ice machine ")
            ON= "close"
        else:
            self.ePushtbtn.setStyleSheet("background-color: red")
            self.mc.publish_to("IOT_PROJECT","Open the Dry-ice machine ")
            ON= "open"
